...
def test_load_audio_pair_strict_mode_preserves_native_audio(tmp_path: Path) -> None:
    input_path = tmp_path / "input.wav"
    target_path = tmp_path / "target.wav"
    input_audio = torch.stack((torch.full((64,), 0.8), torch.full((64,), 0.4)))
    target_audio = input_audio * 0.5
    torchaudio.save(input_path, input_audio, 48_000)
    torchaudio.save(target_path, target_audio, 48_000)

    pair = load_audio_pair(input_path, target_path, strict=True)

    assert pair.input_audio.shape == pair.target_audio.shape == (2, 64)
    torch.testing.assert_close(pair.input_audio, input_audio, atol=1e-4, rtol=0)
    torch.testing.assert_close(pair.target_audio, target_audio, atol=1e-4, rtol=0)
