...
from ..models.recurrent import RecurrentNeuralFXModel
...
class NeuralFXModule(L.LightningModule):
    ...
    def __init__(self, model: RecurrentNeuralFXModel, config: NeuralFXConfig):
        super().__init__()
        self.model = model
        self.config = config
    ...
    def _build_loss(self, loss_config: LossConfig):
        def loss_fn(pred: Tensor, target: Tensor) -> Tensor:
            ...
            if loss_config.weights:
                if loss_config.weights.esr > 0:
                    loss = loss + loss_config.weights.esr * ESR(pred, target)
                if loss_config.weights.mse > 0:
                    loss = loss + loss_config.weights.mse * MSE(pred, target)
            else:
